...
export class LogsService {
    ...
    async findAllLogs(ip?: string, type?: string, user?: string, dateStart?: Date, dateEnd?: Date, module?: string) {
        ...
        if (dateStart !== undefined) {
            // dateStartInGregory = this.common.jalaliToGregory(dateStart)
            dateStartInGregory = dateStart
        }
        if (dateEnd !== undefined) {
            // dateEndIGregory = this.common.jalaliToGregory(dateEnd)
            dateEndIGregory = dateEnd
        }
        ...
        const allLogsChanged = allLogs.map(log => {
            return {
                ...
                // createAt: this.common.gregoryToJalali(log.createdAt),
    ...
    async createLog(log: LogModel) {
        ...
        if (slashindex !== -1) {
            logModule = logModule.slice(0, slashindex)
        }
        if (qsindex !== -1) {
            logModule = logModule.slice(0, qsindex)
        }
